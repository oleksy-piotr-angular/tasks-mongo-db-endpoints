...
export class TodoTaskComponent /*implements OnInit*/ {
  ...
  constructor(private tasksService: TasksService) {
    this.tasksService.getTaskListObs().subscribe((tasks: Array<Task>) => {
      this.tasksList = tasks
        .filter(
          (t) => t.isDone === false
        ) /**this filter method receive Array where 'isDone' = false(Boolean_flag) and return whole Array to local 'this.taskList'*/
        .slice(); //1. after subscribing actual state of 'tasksList' Array<string> from TasksService we assign it to internal variable 'taksList' instead Using @Input() (property-binding) as below(we don't needed to pass data from Parent because Everything is in 'TasksService'):
  ...
  remove(task: Task) {
    ...
    /* //if we select data to be removed  'emit/send' we emit that data from the todo-task.component to the method from Parent by executing this method(look at todo-task.component.html)
