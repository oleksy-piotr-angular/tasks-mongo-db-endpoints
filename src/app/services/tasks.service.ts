...
export class TasksService {
  //everything was moved from 'app.component.ts' component to this 'Service'. TasksService has variables/fields/Arrays below and execute all operations on them but all events receives from child-components|child component deal with the display of data and receiving tasks but not execute any operations
  ...
  /* The BehaviorSubject has the characteristic that it stores the “current” value. This means that you can always directly get the last emitted value from the BehaviorSubject with method getValue(). */
  ...
  constructor(private httpService: HttpService) {
    ...
    //below replace <string> data with <Task> data(object JS) | and after handle HttpService to communicate with Mongo DB, we do not need to initialize whole TasksList |
    /* {
    ...
    //this.tasksListObs.next(tasksList); // 'tasksList' will be emitted to subscribers/we propagate through 'BehaviorSubject'
  ...
  add(task: Task): void {
    ...
    this.tasksListObs.next(this.tasksList); //tasksList was updated above so we emit here its actual content to subscribers */
  ...
  remove(task: Task) {
    //above replace <string> data with <Task> data(object JS)
    /* when we declare a method we need to remember to set appropriate type in parameters*/
    ...
    this.tasksListObs.next(tasksList); //tasksList was removed above so we emit here its actual content to subscribers
    this.removeOneTaskInDB(task); // when we remove some Task in App we also remove it from MongoDB
  }
  done(task: Task) {
    //above replace <string> data with <Task> data(object JS)
    /* when we declare a method we need to remember to set appropriate type in parameters*/
    ...
    this.tasksDoneObs.next(this.tasksDone); //tasksDone was updated above so we emit here its actual content to subscribers
    ...
    /** we do 'const list' because this we cannot inform that some property in Subject was changed we need to again assign the same list */
  ...
  /** we can remove method 'getTaskDoneObs()' which receive 'Observable' data from 'tasksDoneObs' because we use filter 'isDone' when we display each list
   */
