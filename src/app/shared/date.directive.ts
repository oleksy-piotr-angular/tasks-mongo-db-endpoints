...
export class DateDirective {
  ...
  constructor(private el: ElementRef, private renderer: Renderer2) {
    /* above we need to Inject these classes to constructor to manipulate element <li> DOM appearance (and each, where this directive is assigned) and show {{task.end}} and {{task.created}} info from methods and property-binding. It will show after 'Events' in @Hostlistener decorator running methods*/
    this.paragraph = this.renderer.createElement('p'); //create a HTML DOM element <p> tag
  }

  @HostListener('mouseenter') //we initiate @HostListener which listen 'mouseenter' Event and Run method below
  mouseEnter(eventDate: Event): void {
    //this method is running when '@HostListener' receive 'Event'-'mouseenter' and it can be named with another name 'mouseEnter' than Event 'mouseenter' what we See above
    this.renderer.setProperty(
      this.paragraph,
      'innerHTML',
      `Date: ${this.date}` /** remove previous Code because we receive String value only */
    ); //we cannot use 'innerHTML on 'renderer' so we do content replacement to 'this.date' with setProperty() method we change format also with 'localeDateString()' method
